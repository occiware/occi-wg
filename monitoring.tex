...
To specify the details of the monitoring activity the user associates provider-specific \mi s to a \sens\ or \coll\ instance. Three tags are introduced to classify the {\em mixin}s: the {\em Metric} tag is associated with mixins that specify the production of measurement, the {\em Aggregator} tag is associated with mixins that describe how measurements are processed, and the {\em Publisher} tag is associated with mixins that control how results are published. Provider specific {\em Mixin}s are {\em related} to such tags, so to enforce a kind of subtyping. 
