...
\jean{
  Note on core entity attributes rendering, I mention it because of the current implementations.
  IMO, we should avoid multiple representations. The second one is consistent with core.
}

Entity, resource and link attributes defined in OCCI core
specification MAY be represented in the namespace described above or
as top-level attributes in the {\em resource} or {\em link} object.

For instance, the following representations of {\em 'occi.core.target'} attribute of a link
...
\begin{lstlisting}
{
  "links": [
    {
      ...
      "target": "http://example.org/occi/id1",
      ...
    }
  ]
}
...
\jean{Added from discussion on maillist}

The OCCI Collection Format consists of a JSON list containint URIs to entities. Section \ref{sec:example_collection} contains a detailed example.
...
\subsection{Collection Format Example}
\label{sec:example_collection}

\begin{lstlisting}
  [ "http://example.org/id1", "http://example.org/id2" ]
\end{lstlisting}
