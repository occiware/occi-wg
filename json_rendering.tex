...
\end{lstlisting}
\todo{``par'' renamed from ``rel'' -- please check}
...
source of the OCCI Link if this entry is omitted & immutable & 0..1\\
    ...
    par & string & Type identifier of the target Resource, to be supplied if
the target is an OCCI Resource. & immutable & 0..1 \\
	title & String & title of the Link & mutable & 0..1\\
    \botrule
    \end{tabularx}

\flushleft\todo{``par'' renamed from ``rel'' -- please check}
}
...
\end{lstlisting}
\todo{Parent fixed from ``Array'' to ``String'' -- please check}
...
\todo{This needs to be completely redone}
...
\end{lstlisting}
\todo{Added member ``pattern'' -- please check}
