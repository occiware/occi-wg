...
\subsubsection{Rendering of OCCI Resource and Link attributes}
Attributes defined for \hl{Resource} and \hl{Link} types/sub-types
\cite{occi:core} MUST be rendered using the X-OCCI-Attribute HTTP header.  For
...
The following syntax MUST be used to represent OCCI \hl{Resource} and \hl{Link}
attributes:
\begin{verbatim}
...
used to return a list of \hl{Entity} URIs. Each header field value correspond
to a single URI. Multiple \hl{Entity} URIs are returned using multiple
