...
\textbf{Naming: service impl etc shoult be OCCI impl.}
...
\colrule
resource instance & 
Rendering of this instance & 
Create a new instance & 
Trigger action & 
Create an instance at the given path & 
Update an instance at an given path & 
Delete this resource instance \\
\colrule
Path in the namespace hierarchy ending with / &	
Listing of all instances below this namespace & 
Create a new instance & 
N/A & 
N/A & 
N/A & 
Delete all the entries below this namespace hierarchy \\
\colrule
Location of an \hl{Mixin} or \hl{Kind} & 
Listing containing Locations to all entities belonging to this \hl{Mixin} or \hl{Kind} & 
N/A & 
Trigger action (defined for this kind or mixin) on all resource entities belonging to this \hl{Mixin} or \hl{Kind} & 
Add an resource instance to a mixin & 
N/A & 
Delete an instance given in the request from a mixin \\
\colrule
Query Interface	& 
Listing of all registered categories & 
N/A & 
N/A & 
...
be implemented by service providers. Operations which are not defined
are out of scope for this specification and MAY be implemented by the
service provider. This is the minimal set to ensure interoperability.

\subsubsection{Namespace Hierarchy and Location}
The namespace and the hierarchy are free definable by the Service
Provider. The OCCI implementation implement the location path feature,
which is required by OCCI for discovery capabilities and operations on
\hl{Mixin}s and \hl{Kind}s. Location paths tell the client where all
resource instance of one \hl{Kind} or \hl{Mixin} can be found
regardless of the hierarchy the service provider defines. These paths
are discoverable by the client through the Query interface
\ref{sec:query}.
...
\item[Creating a Resource Instance] A request to create a resource
  instance MUST contain at least one Category rendering which is (or
  relates to) a \hl{Kind} definition. If multiple HTTP categories are
...
\end{verbatim}
  ...
  namespace hierarchy.\footnote{If a Service Provider does not want
    the user to define the path of a resource instance it can return a
    Bad Request return code - See section
    \ref{sec:return_codes}. Service Providers MUST ensure that the
    path stay unique in their namespace.}
\begin{verbatim}
...
\item[Retrieving a Resource Instance] For retrieval the HTTP GET verb
  ...
  present. Those \hl{Link}s SHOULD only be the \hl{Actions} which are
  currently applicable. The Attributes of the resource instance MUST
  be exposed to the client if available.
\begin{verbatim}
...
\item[Deleting a Resource Instance] A resource instance can be deleted
  ...
  the request.\footnote{If the resource instances is a \hl{Link} the
    source and target must be updated accordingly}
\begin{verbatim}
...
\item[Triggering an Action on a Resource Instance] To trigger an
  ...
  Category defining the \hl{Action}. It MAY include attributes which
  are the parameters of the action. Actions are triggered using the
  HTTP POST verb and by adding a fragment to the URI. This fragment
  exposes the term of the \hl{Action}. If an action is not available a
...
OCCI. It MUST be found at the path \emph{/-/} off the root of the
service implementation. The following operations, listed below, MUST
...
A Service provider MUST support a filtering mechanism. If a Category
is provided in the request the server MUST only return the complete
rendering of the provided Category.
...
< http://example.com/vms/user2/vm1
\end{verbatim}
\textbf{Note:} A Service provider MUST support a filtering
mechanism. If a Category is provided in the request the server MUST
...
\item[Adding \hl{Mixin} to a Resource Instance to a \hl{Mixin}] One or
...
< http://example.com/vms/user1/vm2
\end{verbatim}
A Service provider MUST support a filtering
mechanism. If a category is provided in the request the server MUST
only return the resource instances belonging to the provided
Category.
...
Extension documents \ref{occi:infrasctucture}.
