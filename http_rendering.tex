...
\begin{description}
  \item[Retrieval of all registered \hl{Kind}s and \hl{Mixin}s] The
    ...
    \hl{Mixin}s the service can handle. This allows the client to
...
\begin{verbatim}
request     = filter
  filter    = *( kind_category_rendering )
              *( mixin_category_rendering ))

response        = categories
  categories    = *( kind_category_rendering )
                  *( mixin_category_rendering ))
\end{verbatim}
...
\end{verbatim}
  ...
  \item[Full update of a \hl{Mixin} or \hl{Kind} collection] A
    ...
    using the HTTP POST verb. All URIs which are part of the
...
\begin{description}
  \item[Creating a resource instance] A request to create a resource
    ...
    instance MUST be used for defining the type of the resource
    instance.  A request MAY also contain one or more HTTP category
    renderings which refer to different \hl{Mixin} instances. Any such
    \hl{Mixin} instances MUST be associated (if allowed) to the
    resource instance.  Optional information which might be provided
    by the client and if available MUST be used are HTTP Links and
    HTTP X-OCCI-Attributes (mapping to \hl{Link} and the attributes of
    an resource instance). Two ways can be used to create a new
    resource instance - HTTP POST or PUT:
...
\end{verbatim}
    ...
    path in the hierarchy of the Service provider's name-space. It
    MUST be the location path of the corresponding \hl{Kind}. The OCCI
    ...
    instance at a certain path in the name-space
...
\end{verbatim}
  ...
  \item[Retrieving a resource instance] For retrieval the HTTP GET
    verb is used. It MUST return at least the HTTP category which
    defines the \hl{Kind} of the resource instance. HTTP Links
...
\begin{description}
  \item[Creation of a \hl{Link} during creation of a \hl{Resource}
    ...
    represent the requested \hl{Kind} MUST be returned - The URIs of
