...
when wanting to represent an empty collection.
...
Category: networkinterface;
          ...
          [...]
    (This is an storage link)\mathbf{
}
Category: my_stuff;
...
More details are discussed in section \ref{sec:content_type}. Other data formats such
...
The structure of these identifiers is opaque and the system should not assume
a static, pre-determined scheme for their structure. For example
\emph{occi.core.id} could be \emph{urn:uuid:de7335a7-07e0-4487-9cbd-ed51be7f2ce4}.
...
implementations MUST, at a minimum, support these verbs as shown in the summary
...
\colrule
resource instance (/vms/user1/vm1) 	& Retrieval of the resource instance's representation 		& Partial update of the resource instance 			& Perform an action 								& Creation/Update of the resource instance, supplying the full representation of the resource instance	& Deletion of the resource instance \\
\colrule
...
AE: Define each and their relationship.

The name-space and the hierarchy are free definable by the Service
Provider. The OCCI implementation MUST implement the location path
feature, which is required by the OCCI Query Interface. Location paths
tell the client where all resource instances of one \hl{Kind} or
\hl{Mixin} can be found regardless of the hierarchy the service
provider defines. Location paths are defined through the HTTP Category
rendering and MUST be present for all HTTP Categories that can be
instantiated (i.e. provisioned). The location paths MUST end with a
\emph{'/'}. These paths are discoverable by the client through the
Query interface \ref{sec:query}.

\subsection{Various Operations and their Prerequisites and Behaviours}
\label{sec:behaviours}
For the expected responses to all the behaviours specified in the sections below,
please refer to the table in section \ref{sec:return_codes}.
...
\begin{description}
  \item[Retrieval of all registered \hl{Kind}s, \hl{Action}s and
    ...
    \hl{Kind}s, \hl{Actions}s and \hl{Mixin}s the service can
...
\end{verbatim}
    ...
    the specified location, scheme, term or rel value the service provider MAY reject this
    operation.
...
AE: Add example that retrieves a collection as well as the example below
...
\begin{description}
  \item[Creating a resource instance] A request to create a Resource
    ...
    Two ways can be used to create a new resource instance - HTTP POST
    or PUT:
...
\end{verbatim}
    ...
    The information which needs to be present in the request and
    response are defined as following and MUST be implemented by an
    OCCI implementation.\footnote{Please note that the Location here
      is defined in RFC2616 \cite{rfc2616} and different from the
      Location definition used in this document (see section
      \ref{sec:render_cats}).}
...
\end{verbatim}
  ...
  \item[Retrieving a resource instance] The HTTP GET verb is used for
    retrieval. It MUST return at least the HTTP category which defines
    ...
    currently applicable SHOULD be rendered using HTTP Links. The
...
\textbf{Resource instances of the type \hl{Link} and \hl{Resource} MUST have their behaviours
  be handled in the same way}. Some exceptions on handling in the
creation and handling of resource instance of the type \hl{Link} are
described in this section. They MUST be implemented by an OCCI
implementation.
...
return code signaling the completion.
...
Return Codes in table. These codes are for behaviours in section \ref{sec:behaviours} where appropriate \ref{tbl:http_codes}
...
%%
%
% TM: all text after this is also Ok after public comment.
%
%%
