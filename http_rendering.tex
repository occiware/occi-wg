...
\end{verbatim}
    ...
    \hl{Kind}s or \hl{Mixin}s.

    The \emph{text/occi} rendering SHOULD be used to define the
    filters in a request.
...
\end{verbatim}
  ...
  \item[Unassociate Resource Instance(s) From a \hl{Mixin}] One or
...
request                    = 1*( resource_representation )
  resource_representation  = 1*( Category )
                             *( Link )
                             *( Attribute )
...
\begin{verbatim}
request                    = 1*( resource_representation )
  resource_representation  = *( Category )
                             *( Link )
                             *( Attribute )
...
\begin{description}
  \item[Inline Creation of a \hl{Link} Instance] When creating a
    ...
    the REST resource which representsthe requested \hl{Kind} MUST be
