...
\item[Removing a \hl{Mixin} definition] A user defined
  \hl{Mixin} MAY be removed (if allowed) by using the HTTP DELETE
...
All of the following operations MUST only be be performed on location paths
provided by \hl{Kind}s and \hl{Mixin}s. The path MUST end with an
\emph{/}.
...
Multiple HTTP header field values MUST be supported as defined by RFC~2616
...
to render multiple header field values, either a comma-separated list or multiple
...
The following example illustrates a rendering of the \hl{Kind} instance
...
instance MUST include a Link header displaying the OCCI \hl{Link} instance
of the relation.
...
i.e.~\hl{Resource} and \hl{Link}, MUST be rendered using the X-OCCI-Attribute
HTTP header. For
example the rendering of a \hl{Compute} instance MUST render the associated
attributes, such as e.g. \texttt{occi.compute.memory}, using X-OCCI-Attribute
headers.

The X-OCCI-Attribute header uses a simple key-value format where each HTTP
header field value represent a single attribute. The field value consist of an
attribute name followed by the equal sign (``='') and an attribute value. The
attribute value MUST be quoted if it contains a separator character as
specified in RFC 2616 (page 16) \cite{rfc2616}.

The following syntax MUST be used to represent OCCI \hl{Entitiy} attributes:
\begin{verbatim}
...
Infrastructure document \cite{occi:infrastructure}.  The rules for defining new
attribute names can be found in the ``Extensibility'' section of the OCCI Core
document \cite{occi:core}.

The following example illustrates a rendering of the attributes defined by
\hl{Compute} type \cite{occi:infrastructure}:
\begin{verbatim}
...
In order to render an OCCI representation solely in the HTTP header, i.e.~using
the \textit{text/occi} content type, the X-OCCI-Location HTTP header MUST be
used to return a list of resource instance URIs. Each header field value correspond
to a single URI. Multiple resource instance URIs are returned using multiple
X-OCCI-Location headers.
...
The following example illustrates the rendering of a list of \hl{Compute}
resource instances:
\begin{verbatim}
...
The responses from an OCCI implementation MAY include headers with
