...
Categories for which resource instance can be reated. The location
...
\item[Retrieval of all registered \hl{Kind}s and \hl{Mixin}s] The HTTP
  verb GET must be used to retrieve all l\hl{Kind}s and \hl{Mixin}s
...
> [...]
> Category: Category: compute; scheme="[...]"; class="action";
...
\item[Associate resource instances with \hl{Mixin}s] One or multiple
...
\end{verbatim}
  ...
  SHOULD be the location path of the corresponding \hl{Kind}. The OCCI
...
> [...]
> Category: compute; scheme="[...]"; class="action"; 
> X-OCCI-Attribute: method=poweroff
...
\item[Creation of a \hl{Link} during creation of a \hl{Resource}
  ...
  resources. Still only the Locaction of the REST resource which
...
\item[Retrieval resource instances of the type \hl{Resource} with
  ...
  directed only those orgininating SHOULD be listed.
\begin{verbatim}
> GET /vms/user1/vm1 HTTP/1.1
> [...]
...
The following syntax MUST be used to represent OCCI \hl{Entitiy}
attributes:
...
calls. Upon long-running operations the OCCI implementation might
return an temporary result and the HTTP 202 return code. Client can
pull upon this resource till the operation finishes. Upon completion
of the operation this temporary result will redirect to the resulting
REST resource using the HTTP 203 return code and a Location header.
