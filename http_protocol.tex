...
\paragraph{Client PUT request}
MUST include a full rendering of the collection.
...
In case server replies with \emph{200 OK} HTTP status code it MUST include either arendering of collection or single entitiy rendering. In case a \emph{201 CREATED} HTTP status code a URI to a entity instance or a list of entity instance URIs MUST be returned.
...
\paragraph{Server POST response}
On successfull operation the server replies with the \emph{200 OK} HTTP status code it MUST include either a rendering of collection or single entity rendering. 
