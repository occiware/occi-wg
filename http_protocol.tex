...
Information about what version of OCCI is supported by an OCCI
implementation MUST be advertised to a client on each response.
...
implements OCCI using a HTTP transport, the \emph{HTTP 501 Not Implemented}
MUST be used.

OCCI implementations which implement this version of this Document
MUST use the version string \emph{OCCI/1.2}. Versioning of extensions
is out of scope for this version of the document.

\subsection{Mandatory HTTP requests headers}
\todo{\textbf{Should this section be called `mandatory' request headers? We are mentioning optional headers as well.}}
