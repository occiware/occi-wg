...
If {\em number} is too large for the provider to handle (policy, technical) then an HTTP 413 response status code MUST be issued to the requesting client.

If there is no more content to be served, the result issued to the requesting client MUST have an HTTP 200 response status code and an empty body.
