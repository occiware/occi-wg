...
Action ``save'' is expected to create an OS Template~\ref{subsubsec:os_tpl} referencing
...
Table~\ref{tbl:ipnetwork} define the attributes introduced by the
\hl{IPNetwork} mixin.
...
\hl{IPNetwork} mixin \hl{Mixin} instance MUST implement these
attributes.
...
Section~\ref{subsubsec:os_tpl}. \hl{Storage} inherits the \hl{Link} base
...
the provider and implementation-specific.
...
If a \hl{Resource} instance carries its own ``size''-related
...
Resource Template. The change must affect the the given \hl{Resource} instance, in
