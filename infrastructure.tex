...
}
{
	...
	save & active (possibly via stop and start chain)\todo{Added ``possibly'', please check} & method=\{hot, deferred\}, name=\emph{String} \\
	\botrule
	\end{tabular}
}
...
environment (e.g., operating system). They MAY, however, choose to define a default resource template that will be used if not explicitly specified. \todo{Please check the previous sentence.} Of the two supported template
types, OS Template is the most basic and necessary template that a provider
SHOULD offer.
