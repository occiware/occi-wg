...
\mytablefloat{\label{tbl:kinds}The \hl{structural-Kind} instances defined for
the infrastructure sub-types of \hl{Resource} and \hl{Link}}{
\begin{tabular}{llll}
Term & Scheme & Title & Related \hl{structural-Kind} \\
\hline
...
The Compute type is assigned the \hl{Kind} instance
...
Compute type. The attributes are defined by the
Compute type's \hl{Kind} instance
\textit{http://schemas.ogf.org/occi/infrastructure\#compute}}{
...
Compute type through its \hl{Kind} instance. These attributes
MUST be exposed by an instance of the Compute type.

\mytablefloat{\label{tbl:compute_actions}%
\hl{Action}s defined by the \hl{Kind} instance
...
Compute type by its \hl{Kind} instance.
% FIXME - The Kind instance is indeed REQUIRED to expose all Actions defined
% but what is required from a Compute instance? Only applicable Actions?
%
% These \hl{Action}s MUST be exposed by an instance of the Compute
% type.
%
