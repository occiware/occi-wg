...
{\bf Ralf: An introduction to the purpose of the Infra document needed here. IaaS blah blah, an example of how a Compute and Storage are associated using a Link etc.}
...
\end{itemize}
{\bf Ralf: Perhaps mention the Link sub-types as well? At least that they exist.}
...
{\bf Ralf: Somewhat unclear which types ``MUST'' be implememented here. Is it the infrastructure types? In that case, exactly which are the infrastructure types? Only Compute/Storage/Network or the Link subtypes as well? Is it really necessary to implement all of them to be compliant? }
...
{\bf Ralf: I think Mixins should receive a more elaborate introduction here as well.}
...
\mytablefloat{\label{tbl:compute}Attributes defined for
	...
	{\bf Ralf: It is a bit confusing that \hl{Category} has an attribute called ``attributes'' which define the attribute names. Possible to make this distinction a bit more clear?}
	}
	{
...
\subsection{Network}
{\bf Ralf: The definition of the Network type found here should be in the intro as well. Intro now only says ``Interconnection resources''.}
...
\hl{IPNetworking} mixin \hl{Mixin} instance MUST expose these attributes.
...
\subsection{Storage}
{\bf Ralf: A definition of Storage needed here.}
...
{\bf Ralf: Explain why Link cannot do so, i.e.~the need for Link attributes.}
...
{\bf Ralf: A short motivation why the StorageLink is necessary would be nice.}
...
{\bf Ralf: Explain what a template is. I.e.~a \hl{Mixin} instance for which the
service provider have defined specific side-effects.}
There are 2 supported infrastructure template types in OCCI.

\subsubsection{OS Template}
{\bf Perhaps a definition before the SHOULD, i.e.~what is an OS Template?}
OCCI implementations SHOULD support this, otherwise what they provision will be merely offer
\hl{Resource}s without any available execution environment (e.g. operating system). Of the two 
