...
\begin{figure}[!h]
	{\centering \resizebox*{0.9\columnwidth}{!}
	    {\includegraphics{figs/infrastructure.pdf}} \par}
...
\mytablefloat{
	\label{tbl:compute}Attributes defined for the \hl{Compute} type. 
}
{
...
\mytablefloat{
	\label{tbl:network}Attributes defined for the \hl{Network} type. 
}
{
	\begin{tabular}{lllll}
	\toprule
...
\hl{IPNetworking} \hl{Mixin} instance MUST implement these attributes.

\mytablefloat{
	\label{tbl:ipnetworking}%
	Attributes defined by the \hl{IPNetworking} \hl{Mixin}. A \hl{Network}
...
\mytablefloat{
	\label{tbl:storage}Attributes defined for the \hl{Storage} type. 
}
{
