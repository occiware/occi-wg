...
using a ``mix-in'' like concept ``Mixins first appeared in the Symbolics' 
object-oriented Flavors\footnote{``Object-oriented programming with flavors'', David A. Moon. Conference on Object Oriented Programming Systems Languages and Applications, 1986} \ref{flavors} system (developed by Howard Cannon), which was an 
approach to object-orientation used in Lisp Machine Lisp.''
\footnote{http://en.wikipedia.org/wiki/Mixin.}

\textbf{(Andy: place references into .bib file)}
...
\hl{Action} type?}
\textbf{(Andy: Yep - that's unclear, hence there is no Action sub-types in infrastructure UML)}

\subsubsection{Mix-ins}
The OCCI Core Model MAY be extended using the mix-in concept
\ralf{Add reference to mix-in concept} 
\textbf{(Andy: references at the top of the doc should be enough now.)}
by defining
