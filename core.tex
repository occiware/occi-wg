...
``meta-model'' \cite{metamodelling}. It is merely a first level
...
using a ``mix-in'' like concept. The mix-in model only applies at the instance
...
\item[resource instance] An instance of a sub-type of \hl{Entity}. The OCCI
 ...
 well.
\item[type] A {\bf type} refer to one of the types defined by the OCCI model.
...
\item The relationship of \hl{Kind} instances reflect the inheritance structure
 starting at the \hl{Entity} type, the parent of all extensions. This makes it
...
organisational purposes. For example a set of operating system templates,
...
sub-type by a submitting a reference to the identifying \hl{Kind}. The
reference SHOULD consist of the term and categorisation scheme which
...
a client SHOULD consist of the term and categorisation scheme which identify
...
The \hl{Action} type is assigned the \hl{Category} identifier
...
Attributes of an OCCI model type instance are
...
The OCCI model MAY be extended using a ``mix-in'' like concept by defining
