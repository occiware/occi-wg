...
Each sub-type of \hl{Entity} is identified by a unique \hl{Type} instance.
%
...
kind & \hl{Entity} & 0..1 & Immutable & \hl{Entity} type uniquely identified by the \hl{Type} instance. \\
\botrule
\end{tabular}
}
...
identifier of a \hl{Entity} sub-type. The following rules apply:
\begin{itemize}
\item A structural \hl{Type} define the capabilities of a \hl{Entity} sub-type in terms of attributes and \hl{Action}s.
...
\item A structural \hl{Type} MUST be related, either directly or indirectly, to
 the structural \hl{Type} of \hl{Entity},
...
\item If type {\bf B} inherit type {\bf A}, where {\bf A} is a sub-type of
 \hl{Entity}, the structural \hl{Type} of {\bf B} MUST be directly related to the
 structural \hl{Type} of {\bf A}.
\end{itemize}

\paragraph*{Non-structural Type} A non-structural \hl{Type} is an instance of
\hl{Type} {\em not} assigned as the unique identifier of any \hl{Entity} sub-type.
The following rules apply:
\begin{itemize}
\item A non-structural \hl{Type} define additional capabilities for each
 \hl{Entity} sub-type instance it is associated with. A non-structural \hl{Type}
 add capabilities using a mix-in like model.
\item A non-structural \hl{Type} MUST only be associated with \hl{Entity}
...
\item A non-structural \hl{Type} MUST NOT be related, neither directly nor
 indirectly, to the structural \hl{Type} of \hl{Entity},
 i.e.~\textit{http://schemas.ogf.org/occi/core\#kind}.
 %
 See section~\ref{sec:type_relationship} for the definition of \hl{Type}
 relationship.
...
either directly or indirectly, to the structural \hl{Type} of \hl{Entity},
i.e.~\textit{http://schemas.ogf.org/occi/core\#kind}.
%
...
\hl{Entity} defined by an implementation. A \hl{Entity} sub-type instance MUST be
...
One or more \hl{Entity} sub-type instances associated with the same \hl{Type},
...
different \hl{Entity} sub-type instances associated with the \hl{Type}.

A \hl{Entity} sub-type instance is always a member of the \hl{Entity}'s structural
\hl{Type} collection since a \hl{Entity} sub-type instance MUST be associated
with the structural \hl{Type} of the \hl{Entity} sub-type.
Since a non-structural \hl{Type} can be assigned to any \hl{Entity} sub-type
...
corresponding non-structural \hl{Type} to the \hl{Entity} sub-type instance.
\item[Removing an instance] from a collection is accomplished by disassociating the
corresponding non-structural \hl{Type} from the \hl{Entity} sub-type instance.
\end{description}
%
...
%
Table~\ref{tbl:kind} defines the attributes the \hl{Entity} type MUST implement to
be compliant.

\mytablefloat{\label{tbl:kind}Attributes defined for the \hl{Entity} type.}{
...
%
Every sub-type of \hl{Entity} MUST be assigned a structural \hl{Type}, see
section~\ref{sec:type}. \hl{Entity} itself is assigned the structural \hl{Type}
\textit{http://schemas.ogf.org/occi/core\#kind}.
%
A \hl{Entity} sub-type instance MAY be associated with one or more non-structural
\hl{Type}s.

A \hl{Entity} sub-type instance MUST expose its structural \hl{Type} and any
...
The \hl{Action} type defines an invocable operation applicable to a \hl{Entity}
