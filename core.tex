...
implemented.
Instances of the \hl{Category} type itself are only used
...
\mytablefloat{
  ...
  \begin{tabular}{lllp{1.3cm}p{1.0cm}p{7cm}}
    ...
    \colrule
    id			& UUID 		& 1 		& Immutable 		& Yes 			& A unique identifier (within the service provider's name-space) of the \hl{Entity} sub-type instance. \\
...
%
\mytablefloat{
  \label{tbl:entity}OCCI Attributes defined by the \hl{Entity} type.}{
  \begin{tabular}{lllp{1.3cm}p{1.0cm}p{7cm}}
    \toprule
...
An \hl{Action} can expose OCCI Attributes which correspond to arguments of
