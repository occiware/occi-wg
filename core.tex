...
% header
\fancyhf{}
\lhead{Errata Draft GFD-P-R.183}
\rhead{\today}
...
\rightline {Alexander Papaspyrou, TU Dortmund University}
\rightline {Thijs Metsch, Platform Computing}
\rightline {April 7, 2011}\\
\rightline {Errata Update: \today}
...
\mytablefloat{
  ...
  \begin{tabular}{llllp{2.7in}}
    ...
    attributes		& \hl{Attribute} & 0..* 		& Immutable 			& Set of \hl{Attribute} instances. \\
    \botrule
  \end{tabular}
}
...
\mytablefloat{
  ...
  \begin{tabular}{lp{23mm}llp{60mm}}
    ...
    type		& Enum \{string, number, boolean\}
    					& 0..1 		& Immutable 			& OCCI Attribute type. \\
...
\mytablefloat{
  ...
  \begin{tabular}{llllp{2.7in}}
    ...
    related 		& \hl{Kind} 	& 0..1 			& Immutable 			& Another \hl{Kind} instances which this \hl{Kind} relates to. \\
...
resource instance and thereby identify new resource capabilities,
i.e.~\hl{Attribute}s and \hl{Action}s, for the resource instance.
However, a
...
\begin{itemize}
  ...
  \item A \hl{Mixin} instance is {\em only} a type identifier. It MUST NOT
    provide the implementation of the new resource capabilities it introduces.
...
sub-types is called a {\em resource instance}.
Each resource instance within an OCCI system MUST have a unique identifier%
\footnote{A resource instance identifier MUST be unique within the service
...
Although every unique resource instance identifier MUST be valid URI it is
...
\mytablefloat{
  ...
  \begin{tabular}{lllp{1.3cm}p{1.0cm}p{7cm}}
    ...
    mixins 		& \hl{Kind}	& 0..* 		& Mutable 		& No 			& The \hl{Mixin} instances associated to this resource instance. Consumers can expect the \hl{Attribute}s and \hl{Action}s of the associated \hl{Mixin}s to be exposed by the instance. \\ 
    \botrule
  \end{tabular}
}
...
\item Clarify the format of the unique resource instance identifier defined in
