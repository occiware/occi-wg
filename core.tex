...
\mytablefloat{
  ...
  \begin{tabular}{ll}
    ...
    attributes 		& Attribute({\tt "size"}) \todo{Please check if this fix is right}\\
  \botrule
  \end{tabular}
}
...
\todo{Please check if it makes sense to add something like this. It provides a basis for replacing Resource templates :}\\
The handling of templates with a common (transitive) parent \hl{Kind}, if assigned repeatedly, subsequently, during run time, MAY be defined case-by-case. A new template may, e.g., replace the previous one, or be rejected. An example of this is the definition of replacing Resource Templates in~\cite{occi:infrastructure}.
...
type identified by the \hl{Kind}. \todo{Please check that removing parentheses from here was correct.}
...
\label{sec:entity}
The \hl{Entity} type is an abstract parent\todo{ Check if this change from ``type'' to ``parent'' (or ``parent type''?) is correct} of the \hl{Resource} type and
...
\mytablefloat{
  ...
  \begin{tabular}{llllp{7.5cm}}
    ...
    mixins 		& \hl{Kind}	& 0..* 		& Mutable 		& The \hl{Mixin} instances associated to this entity instance. Consumers can expect the \hl{Attribute}s and \hl{Action}s of the associated \hl{Mixin}s to be exposed by the instance. \\
    \botrule
  \end{tabular}
}
...
\newpage
\section{Change Log}\todo{This is a list of changes, not Errata. But check and decide}
\label{sec:errata}
...
nor on invocation of \hl{Action}s in existing implementations. The OCCI HTTP Rendering
