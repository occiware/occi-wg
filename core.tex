...
The corrections introduced by the {\today} errata update is summarized below.
...
\item Rename the term {\em resource instance} to {\em entity instance}.
  An {\em entity instance} refers to a instance of either OCCI \hl{Resource},
...
The \hl{Attribute} type formalize how attribute properties are represented in
...
OCCI Infrastructure \cite{occi:infrastructure} define attribute properties for
...
OCCI HTTP Rendering \cite{occi:http_rendering} already expose the ``required'' and
``mutable'' attribute properties. Future updates may include support for all optional
...
%
The OCCI Infrastructure \cite{occi:infrastructure} specification only define
discoverable OCCI \hl{Attribute} although this is not explicitly stated.

\subsection{Action definition}

The corrected definition of OCCI \hl{Action} have no impact on neither discovery
...
since it already use {\tt type="action"} in its rendering of categories.
