...
\begin{description}
  ...
  \item[mixin] An instance of the \hl{Mixin} type associated with an
    {\bf entity instance}. The \textbf{mixin} concept as used by OCCI
...
(including associated behaviours) can interact with it and extensions
...
The mixin model only applies at the instance level, i.e., the ``object
level'', and thereby differs from the more common uses of the mixin
concept. A mixin in OCCI can never be applied to a type, only to an
instance.
...
instance to mixin additional capabilities at run-time.
...
\mytablefloat{
  ...
  \begin{tabular}{llllp{7.5cm}}
    ...
    kind 		& \hl{Kind}	& 1 		& Immutable 		& The \hl{Kind} instance uniquely identifying the particular \hl{Entity} sub-type of this instance. \\
    mixins 		& \hl{Mixin}	& 0..* 		& Mutable 		& The \hl{Mixin} instances associated to this entity instance. Consumers can expect the \hl{Attribute}s and \hl{Action}s of the associated \hl{Mixin}s to be exposed by the instance. \\
    \botrule
  \end{tabular}
}
...
\mytablefloat{
  ...
  \begin{tabular}{llllp{7.5cm}}
    ...
    source.type    & \hl{Kind} & 0..1     & Mutable     & The \hl{Kind} of {\tt source}. \\
    target 		& \hl{URI} & 1 		& Mutable 		& The unique identifier of an \hl{Object} this \hl{Link} instance points to. \\
    target.type    & \hl{Kind} & 0..1    & Mutable     & The \hl{Kind} of {\tt target}, if applicable. \\
    \botrule
  \end{tabular}
}
...
hosted by a third-party. {\tt source.type} and {\tt target.type} MAY be used to explicitly define the \hl{Kind} of the \hl{Resource}
instance in {\tt source} and {\tt target} respectively.
...
provider-specific category instances, sub-typing and mixins. Custom sub-typing requires provider-specific
\hl{Kind} instances and custom mixins require provider-specific
...
The OCCI Core Model MAY be extended using a mixin like concept by
...
\begin{itemize}
\item Added optional typed \hl{Link} (via {\tt source.type} and {\tt target.type}
  attributes).
