...
Attribute          = "X-OCCI-Attribute" ":" #attribute-repr
  ...
  attribute-value  = ( string | number | bool | enum-val | array | hash )
  ...
  enum-val         = string
  array            = "[" *array-elm "]"
  array-elm        = ( string | number | bool | enum-val ) ","
  hash             = "{" *hash-elm "}"
  hash-elm         = hash-key ":" "hash-value" ","
  hash-key         = string
  hash-value       = ( string | number | bool | enum-val )
\end{verbatim}
...
\begin{itemize}
  \item Added support for complex \hl{Attribute} values {\tt List} and {\tt Hash}. Both
        rendered into text as string representations of {\tt Array} and {\tt Hashmap} respectively.
