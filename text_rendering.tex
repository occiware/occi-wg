...
This document presents the text based renderings. To be complaint OCCI implementations MUST implement three renderings defined in sections \ref{sec:text}, \ref{sec:header} and \ref{sec:urilist}.

The document is structures by defining based ABNFs which can then be combined to renderings which will be rendered using HTTP by the specific rendering definitions.

\section{ABNF Definitions}

For the following section \ref{sec:renderings} these ABNF notations will be used. Implementations MUST hence implement the renderings according to these definitions.

\subsection{Category rendering}
...
While using this rendering the renderings MUST be placed in the HTTP Body. Each entry consists of a name followed by a colon (”:”) and the field value.
...
\section{URI listening rendering}
\label{sec:urilist}
